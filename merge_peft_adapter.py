...
MODEL_CLASSES = {
    ...
    "chatglm": (AutoModel, ChatGLMTokenizer),
...
def main():
    ...
    word_embeddings = base_model.get_input_embeddings().word_embeddings if hasattr(base_model.get_input_embeddings(), 'word_embeddings') else base_model.get_input_embeddings()
    base_model_token_size = word_embeddings.weight.size(0)
    '''
    if base_model_token_size != len(tokenizer):
        print("base_model:{}".format(type(base_model)))
